...
export function SavedElementCalculation({ element, setting, result, onChange }: { element: TechnicalElement; setting: CalculationSetting; result: ElementErgebnis; onChange: (setting: CalculationSetting) => void }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
    ...
    <div className="border-t border-neutral-100 p-4"><div className="grid gap-4 xl:grid-cols-[2fr_1fr]">
      ...
      <label><span className="mb-1.5 block text-xs font-bold text-neutral-600">Zusätzlicher Zuschlag pro Element (€)</span><input type="number" min="0" step="0.01" value={setting.surcharge} onChange={(event) => nonNegative(event.target.value) && patch({ surcharge: event.target.value })} placeholder="0,00" className={inputClass} /></label>
    </div></div>
    <footer className="grid grid-cols-2 gap-3 border-t border-neutral-100 bg-neutral-50 p-4 sm:grid-cols-4"><Result label="Laufende Meter" value={result.gueltig ? `${meters.format(result.laufendeMeterGesamt)} m` : '—'} /><Result label="Einkauf" value={result.gueltig ? currency.format(result.einkaufGesamt) : '—'} /><Result label="Verkauf" value={result.gueltig ? currency.format(result.verkaufGesamt) : '—'} /><Result label="Gewinn" value={result.gueltig ? currency.format(result.gewinnGesamt) : '—'} highlight /></footer>
  </article>
}
